...
class InvertedPendulumWidget:
    ...
    """
    def __init__(self, _loop_id, parent=None):
        ...
        self.state4plot = []
        self.state = [self.freshest_state]
        self.time_step = [self.freshest_timestep]
        if self.draw_flag == GUIShowType.animation:
            self._init_animation()
            self.update_draw = self.update_animation
        elif self.draw_flag == GUIShowType.realtime_plot:
            self._init_static_plot()
            self.update_draw = self.update_2d_plot
    ...
    def _init_animation(self, width=6, height=6, dpi=100):
        try:
            ...
            self.fig = plt.figure(figsize=(width, height), dpi=dpi)  # Matplotlib Figure
        ...
        self._animation_ax = self.fig.add_subplot(111, autoscale_on=False, xlim=(-1., 1), ylim=(-1., 1))
        ...
        self.cart_marker, = self._animation_ax.plot([0], [0], linestyle='None', marker='s',
                                                    markersize=40, markeredgecolor='k',
                                                    color=loop_color, markeredgewidth=2)

        # Round marker for the head of the pendulum
        self.pendulum_marker, = self._animation_ax.plot([0], [0], linestyle='None', marker='o',
                                                        markersize=20, markeredgecolor='k',
                                                        color=loop_color, markeredgewidth=2)

        # Line for the pendulum's body
        self.pendulum_line, = self._animation_ax.plot([0], [0], 'o-', color=loop_color, lw=4,
    ...
    def update_state(self, timestep: int, state) -> bool:

        if timestep > self.freshest_timestep:
            self.freshest_state[:, 0] = state[:, 0]
            self.freshest_timestep = timestep
            real_time_s = timestep * SAMPLING_PERIOD_S
            self.state = np.append(self.state, [state], axis=0)
            self.time_step = np.append(self.time_step, [real_time_s], axis=0)
#            self.state4plot = self.state2coord(self.freshest_state) # TODO remove this state4plot completely
    ...
    def _get_pendulum_head_coordinate_x(self, l = 0.5) -> float:
        # phi positive in counterclockwise
        return -l * np.sin(self.freshest_state[PlantStates.pendulum_angle, 0]) + self._get_cart_coordinate_x()

    def _get_pendulum_head_coordinate_y(self, l = 0.5) -> float:
        return l * np.cos(self.freshest_state[PlantStates.pendulum_angle, 0])
